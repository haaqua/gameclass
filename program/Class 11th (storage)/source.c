...
#pragma region Data 영역
	...
	// 프로그램이 종료될 때 메모리가 해제되는 영역입니다

#pragma endregion
#pragma region BSS 영역
	// 프로그램이 실행될 때 초기화가 이루어지지 않은 
	// 전역 변수와 정적 변수가 저장되는 메모리 영역입니다

#pragma endregion
#pragma region Stack 영역
	// 프로그램이 자동으로 사용되는 임시 메모리 영역으로
...
#pragma region 지역 변수
	// 함수 내부에서 생성된 변수로 함수 내부에서만 접근할 수 있으며,
...
#pragma region 정적 변수
	// 지역 변수의 특성을 가지면서 전역 변수의 특성을 가지고 변수로, 단 한 번만 초기화가 이루어지며, 
	// 프로그램이 실행될 때 메모리에서 생성되고, 프로그램이 종료되면 메모리에서 해제되는 변수입니다
